...
async function getCriticalFormula(formula, rollData) {
    return new CONFIG.Dice.DamageRoll(formula, rollData, {critical: true, powerfulCritical: game.settings.get('ars', 'criticalDamageMaxDice'), multiplyNumeric: game.settings.get('ars', 'criticalDamageModifiers')}).formula;
}
